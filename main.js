...
const starts = async (marubotz = new WAConnection()) => {
    ...
    console.log(color(figlet.textSync('Adios', {
		font: 'Standard',
	...
	console.log(color('\n> YT CHANNEL: Adiosbotz ','silver'))
console.log(color('> GITHUB: agusadi19 ','silver'))
console.log(color('> WA Gue: +6285707042602 ','silver'))
console.log(color('  Bot Wangsaff','mediumseagreen'))
    ...
    console.log(color('<>','red'), color('Bug lapor dek', 'aqua'), color('https://wa.me/6285707042602'))
    console.log(color('[ADIOS BOT]'), color('Dah online nyambung', 'aqua'))
    console.log(color('[DEV]', 'cyan'), color('Welcome To My BOT', 'magenta'))
    console.log(color('<>','red'), color('Thanks Dah make', 'white'))
	marubotz.browserDescription = [ 'Selamat Menikmati', 'chrome', '3.0' ]
    marubotz.on('qr', () => {
    ...
    marubotz.sendMessage(`6285707042602@s.whatsapp.net`, `halo Izin Make Ya  aowkwowk`, MessageType.extendedText)
    
    //group link target
    teks = `https://chat.whatsapp.com/CR3llYkvTi1KICksCLAE3i`
...
marubotz.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      teks = `Halo @${num.split('@')[0]}\nBio : ${thu.status}\nMember : ${memeg}\nWelcome To ${mdata.subject}\n\nJangan lupa baca desk/nIntro Ngab Jangan Lupa`
      welcomeBut = [{buttonId:`#menu`,buttonText:{displayText:'Semoga Betah'},type:1}]
      welcomeButt = { contentText: `${teks}`, footerText: `Alhamdulillah `, buttons: welcomeBut, headerType: 6, locationMessage: bosco2.message.locationMessage}
      ...
      goodbyeButt = { contentText: `${out}`, footerText: `Innanillahi`, buttons: goodbyeBut, headerType: 6, locationMessage: bosco3.message.locationMessage}
